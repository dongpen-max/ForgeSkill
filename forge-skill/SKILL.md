...
Use this skill to turn a broad software idea into evidence-grounded output by surveying GitHub candidates, reranking them for domain fit, extracting strengths and gaps, and synthesizing a new project or Codex skill.

## Workflow

1. Define the target domain and output.
   - Use the user's phrase as the domain.
   - If the user says "skill", create or update a Codex skill.
...
5. Synthesize the new output.
   ...
   - For a new Codex skill, follow local skill conventions: concise `SKILL.md`, optional scripts/references/assets only when reusable, `agents/openai.yaml`, and validation with the skill creator validator if available.
...
6. Materialize files when requested.
   ...
   - For a skill, default the output directory to the user's personal Codex skills directory when the user has not specified another location.
